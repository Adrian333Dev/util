...
const { summary } = require('./command');
const sources = require('./sources');

/** Names `util` answers itself. A namespace called one of these is unreachable. */
const RESERVED = ['ls', 'install', 'uninstall', 'source', 'help'];

/** Never a command: a dotfile, such as `.alias`, or documentation. */
const SKIP = /^\.|\.(md|txt)$/i;

/** The short name a namespace folder gives itself in `.alias`, or null. */
function readAlias(dir) {
  try {
    return fs.readFileSync(path.join(dir, '.alias'), 'utf8').trim() || null;
  } catch {
    return null;
  }
}
...
function build() {
  ...
  for (const source of sources.all()) {
    const found = { ...source, missing: !fs.existsSync(source.path), namespaces: [] };
    listing.push(found);
    ...
    for (const nsName of entries(source.path, 'dir')) {
      ...
      const ns = {
        name: nsName,
        alias: readAlias(nsDir),
        reserved: RESERVED.includes(nsName),
        commands: [],
      };
      found.namespaces.push(ns);

      // The first source holding a namespace names it. A second source adding
      // commands to the same namespace inherits the alias rather than
      // competing for it.
