...
function add(typed) {
  ...
  if (!fs.existsSync(resolved)) {
    throw new UtilError(`${resolved} does not exist, so there are no commands to read from it.`);
  }
 ...
 */
function all() {
  const sources = registered().map((p) => ({ path: p, type: 'registry' }));
  const project = projectSource();
  if (project && !sources.some((s) => s.path === project)) {
    sources.push({ path: project, type: 'project' });
  }
  return sources;
}
