 ...
 *   UPSTREAM_URL=... util claude proxy    forward to another Anthropic-compatible API
 *   PROXY_LOGS=/tmp/x util claude proxy   write the documents somewhere else
 *
 * Then, in the terminal you run Claude Code from:
 *
 ...
 *   faster than usual until it settles.
 *
 * Not written here. Matt Pocock published it as `agent-proxy`:
...
// A module, not a script: `require` reaches lib/ the same way the others do.
createRequire(import.meta.url)("../../lib/command").helpOrRun(
...
/** Any Anthropic-compatible endpoint, not only api.anthropic.com. */
const UPSTREAM = new URL(process.env.UPSTREAM_URL ?? "https://api.anthropic.com");
const UPSTREAM_HTTP = UPSTREAM.protocol === "http:";
const UPSTREAM_PORT = UPSTREAM.port || (UPSTREAM_HTTP ? "80" : "443");

/**
 * Beside you, not beside the script. This file lives wherever util was cloned,
 * and a command on PATH writing into its own source folder is a surprise.
 */
const LOG_DIR = process.env.PROXY_LOGS ?? path.join(process.cwd(), "logs");
...
function handle(req, res) {
  ...
  req.on("end", () => {
    ...
    const upstream = (UPSTREAM_HTTP ? http : https).request(
      {
...
http.createServer(handle).listen(PORT, () => {
  ...
  console.log(`[agent-proxy] forwarding to ${UPSTREAM.origin}`);
  console.log(`[agent-proxy] documents land in ${LOG_DIR}`);
  console.log(
